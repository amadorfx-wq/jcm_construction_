// ─────────────────────────────────────────────────────────────────────────────
// [locale] Silo: Remodelación de Cocinas — Server Component
// ─────────────────────────────────────────────────────────────────────────────
...
export async function generateMetadata({
  ...
  return {
    title: isEs
      ? 'Remodelación de Cocinas High-End | JC Milian Construction'
      : 'High-End Kitchen Remodeling | JC Milian Construction',
    description: isEs
      ? 'Cocinas de lujo en Atlanta, GA. Mármol Calacatta, gabinetes Shaker, electrodomésticos integrados.'
      : 'Luxury kitchens in Atlanta, GA. Calacatta marble, Shaker cabinets, integrated appliances.',
  };
}

export default async function RemodelacionCocinasPage({
  params,
}: {
  params: Promise<{ locale: string }>;
}) {
  ...
  const silo = dict.silos.kitchens;
  const project = getProjectsByType('cocina', locale)[0];
